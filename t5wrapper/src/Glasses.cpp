...
T5_Result Glasses::ConfigureWandStream(bool enabled)
{
	T5_WandStreamConfig config = {};
	config.enabled = enabled;
	return t5ConfigureWandStreamForGlasses(handle, &config);
}

OptWandStreamEvent Glasses::ReadWandStream(uint32_t timeoutMs)
{
	T5_WandStreamEvent event = {};
	auto err = t5ReadWandStreamForGlasses(handle, &event, timeoutMs);
	return err == T5_SUCCESS ? OptWandStreamEvent(event) : OptWandStreamEvent(err);
}
